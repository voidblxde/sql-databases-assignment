-- Задача 2: Рекурсивное дерево + кол-во задач и прямых подчиненных
WITH RECURSIVE EmpTree AS (
    SELECT EmployeeID, Name, ManagerID, DepartmentID, RoleID
    FROM Employees WHERE EmployeeID = 1
    UNION ALL
    SELECT e.EmployeeID, e.Name, e.ManagerID, e.DepartmentID, e.RoleID
    FROM Employees e JOIN EmpTree et ON e.ManagerID = et.EmployeeID
)
SELECT
    et.EmployeeID,
    ...
    (SELECT COUNT(*) FROM Employees sub WHERE sub.ManagerID = et.EmployeeID) AS TotalSubordinates
FROM EmpTree et
